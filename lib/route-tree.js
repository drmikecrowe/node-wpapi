...
function reduceRouteComponents( routeObj, topLevel, parentLevel, component, idx, components ) {
	...
	let groupPattern = namedGroup && namedGroup[ 2 ];
	// This catches nested parens that aren't matched properly
	if (namedGroup && namedGroup.length > 3 && namedGroup[3].indexOf("?)") > -1) {
    groupPattern += namedGroup[3];
	}
